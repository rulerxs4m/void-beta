...
def datewise_plotter(dates: list[datetime.date], values: list[int], *, max_xticks: int = 15, filename: str = "plot.png"):
    ...
    # Create a PCHIP interpolator for smooth curve
    ...
    plt.stackplot(x_smooth, y_smooth, colors=["white"], alpha=0.8)
    plt.plot(x, y, "o", color="white", markerfacecolor="white", markeredgecolor="white")
    ...
    ax.set_ylim(-max(y) * 0.10, max(y) * 1.15)
