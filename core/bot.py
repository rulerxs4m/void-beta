...
class GiftifyHelper:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Initialize early attributes
        self.cached_giveaways = []  # or dict if you're using message_id as keys
        
    async def setup_hook(self):
        # Create DB pool first
        self.pool = await asyncpg.create_pool(
            user="user", password="pass", database="dbname", host="localhost"
        )
    async def send(
        self,
